public class ArrayList {
	...
	public void insert (int index, int data) {
		...
		int prev = this.list[index];
		this.list[index] = data;

		for (int i = index + 1; i < this.length + 1; i++) {
			int tmpPrev = this.list[i];
			this.list[i] = prev;
			prev = tmpPrev;
		}
	...
	public int pop (int index) {
		...
		int data = this.list[index];
		int next = this.list[index + 1];
	...
	// Clear the list.
	public void clear () {
		this.list = new int[this.initalSize];
		this.length = 0;
	}
	...
	public void reverse () {
		for (int i = 0; i < this.length / 2; i++) {
			...
			this.list[this.length - i - 1] ^= this.list[i];
			this.list[i] ^= this.list[this.length - i - 1];
		}
	}
